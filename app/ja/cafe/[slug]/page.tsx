...
export function generateMetadata({ params }: { params: { slug: string } }): Metadata {
  ...
  return {
    ...
    description: `${name}（${v.nearestStation}周辺）の作業向け情報。${wifi}・${power}。確認日つきでご紹介します。`,
    alternates: {
...
export default function CafePageJa({ params }: { params: { slug: string } }) {
  ...
  const faq = [
    ...
    },
    {
      ...
      a: v.laptopFriendly
        ? `はい。${v.hasWifi ? "Wi-Fi" : "Wi-Fiなし"}・${v.hasPower ? "電源あり" : "電源なし"}で、${v.nearestStation}の近くで作業に使えます。`
  ...
  return (
    <div>
      ...
      <p className="lede" style={{ marginTop: 14 }}>
        {v.nearestStation}から徒歩{v.walkMinutes}分。{wifiTxt}・{powerTxt}。
      </p>

      <div className="facts">
        <div><span className="k">最寄り駅： </span>{v.nearestStation}（徒歩{v.walkMinutes}分）</div>
